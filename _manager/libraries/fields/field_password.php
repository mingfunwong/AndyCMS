...
class field_password {
    ...
    public function on_info($data) {
        return array('type' => 'VARCHAR', 'constraint' => from($data, 'length', 255), 'default' => '');
    }
    /**
     * 生成字段的表单控件
     *
    ...
    public function on_form($field, $default = '', $has_tip = TRUE) {
        echo '<input class="form-control" name="' . $field['name'] . '" id="' . $field['name'] . '" type="password" autocomplete="off" value="' . $default . '">';
    ...
    public function on_do_post($field, &$post) {
        $CI = ci();
        $post[$field['name']] = $CI->input->post($field['name'], true);
    }
}

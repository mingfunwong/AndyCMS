...
class User extends Admin_Controller {
    ...
    public function view($role = 0) {
        ...
        $config['base_url'] = backend_url('user/view') . '?cms';
        $config['per_page'] = 15;
        ...
        $data['bread'] = make_bread(Array('用户管理' => '',));
    ...
    public function _add_post() {
        $data['roles'] = $this->user_mdl->get_roles();
        $data['bread'] = make_bread(Array('用户管理' => backend_url('user/view'), '增加用户' => '',));
    ...
    public function _edit_post($id = 0) {
        ...
        if (!$this->_validate_user_form($data['user']->username, TRUE)) {
            $data['bread'] = make_bread(Array('用户管理' => backend_url('user/view'), $data['user']->username => '',));
    ...
    private function _validate_user_form($name = '', $edit = FALSE) {
        ...
        $this->form_validation->set_rules('username', '用户名称', 'trim|required|min_length[2]|max_length[16]' . $callback);
